...
def save_gifti(data, save_name, hemisphere=None):
    """
    ...
    nvPair = gifti.GiftiNVPairs(name='AnatomicalStructurePrimary', value=hemisphere)
    metaData = gifti.GiftiMetaData(nvPair)

    data = np.squeeze(data.astype(np.float32))
    img = gifti.GiftiImage(meta=metaData)
    gda = gifti.GiftiDataArray(data=data)
